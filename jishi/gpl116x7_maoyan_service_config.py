...
class TelnetClient:
    ...
    @outer
    def service_add_uni(self):
        self.login_host()
        
        cn=sys._getframe().f_code.co_name
        cmds=['no gpon-onu-service-profile 121-125',
              'show gpon-onu-service-profile all',
              'gpon-onu-service-profile 121',
              'port-num ethernet 4 pots 1 veip 1',
              'commit',
              'exit',
              'show gpon-onu-service-profile all',
              'show gpon-onu-service-profile 121 | section-end wlan']
        #检查单个service是否创建成功···
        check_name="tips:检查单个service是否创建成功......"
        check_words=['ServiceProfile Name: profile-121','Ethernet  4','POTS      1','VEIP      1']
        output_lst=[]
        self.check_res1(cn,cmds,check_name,check_words,output_lst)
    ...
    @outer
    def service_del_uni(self):
        self.login_host()

        cn=sys._getframe().f_code.co_name
        cmds=['show gpon-onu-service-profile all',
              'no gpon-onu-service-profile 121',
              'show gpon-onu-service-profile all']
        #检查单个service是否删除成功···
        check_name='tips:检查单个service是否删除成功......'
    ...
    @outer
    def service_del_mul(self):
        self.login_host()
        
        cn=sys._getframe().f_code.co_name
        cmds=['show gpon-onu-service-profile all',
              'no gpon-onu-service-profile 121-122',
              'show gpon-onu-service-profile all']
        #检查多个service是否删除成功···
        check_name='tips:检查多个service是否删除成功......'
    ...
    def service_show(self):
        ...
        cn=sys._getframe().f_code.co_name
        cmds=['show gpon-onu-service-profile all']
        #检查service-profile 123是否存在···
        check_name='tips:检查service-profile 123是否存在......'
    ...
    def clear_config(self):
        ...
        cn=sys._getframe().f_code.co_name
        cmds=['no gpon-onu-service-profile 121-125','show gpon-onu-service-profile all']
        #检查配置service-profile 121-125是否清除···
        check_name='tips:检查配置service-profile 121-125是否清除......'
        check_words=['121         profile-121',
                     '122         profile-122',
                     '123         profile-123',
                     '124         profile-124',
...
if __name__ == '__main__':
    ...
    telnet.check_onu()
    telnet.service_add_uni()
    telnet.service_del_uni()
    telnet.service_add_mul()
    telnet.service_del_mul()
    telnet.service_show()
    telnet.clear_config()
    #将标准输出和标准错误保存到log文件  
